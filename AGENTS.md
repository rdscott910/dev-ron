...
- Content routes include `/resume` (career timeline from `src/lib/career.ts` with sanitized PDF download), `/now`, `/uses`, `/methodology` (AI orchestration hub, wired as the case-study link on the AI Orchestration Server project card), and `/projects/benefits-eligibility-agent` (NC SNAP Benefits Eligibility Agent case study; live demo at `demo.dev-ron.com` / `benefits-eligibility-agent.vercel.app`).
...
- Static content data sources: `src/lib/projects.ts` (typed `Project` records), `src/lib/blog.ts` (typed `Post` records), and `src/lib/career.ts` (career timeline); real seeded projects are the NC SNAP Benefits Eligibility Agent (public GitHub: `rdscott910/Benefits-Eligibility-Agent`, live demo + case study), the AI Orchestration Server (public GitHub: `rdscott910/ai-orchestration-server`), and the FQB accounting platform (no public link).
